...
def display_info():
    # Display information
    print("EPS = " + str(_EPS))
    print("ATTACK_RATE = " + str(_RATE))
    print("VIDEO_PATH = " + _DATA_ROOT['ucf101']['rgb'])
    print("------------------------------------------")

def main(dataset, mode, split):
    assert mode in ['rgb', 'flow', 'mixed']
    log_dir = os.path.join(_LOG_ROOT, 'test-%s-%s-%d-fgsm-01-rate-%d' % (dataset, mode, split, _RATE))
    ...
    if mode in ['rgb', 'mixed']:
        rgb_data = ActionDataset(
            dataset, class_num, test_info_rgb, 'img_{:05d}.jpg', mode='rgb')
